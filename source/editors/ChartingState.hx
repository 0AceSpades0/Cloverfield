...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var erectCheck = new FlxUICheckBox(10, player2DropDown.y + 5 + 40, null, null, "Erect Mode?", 100);
		var picoCheck = new FlxUICheckBox(erectCheck.x, erectCheck.y + 20, null, null, "Pico Mix?", 100);
		var bfCheck = new FlxUICheckBox(picoCheck.x, picoCheck.y + 20, null, null, "Bf Mix?", 100);


		erectCheck.checked = _song.isErect;
		// _song.needsVoices = check_voices.checked;
		erectCheck.callback = function()
		{
			_song.isErect = erectCheck.checked;
			_song.isPico = false;
			_song.isBf = false;
			picoCheck.checked = false;
			bfCheck.checked = false;
			//trace('CHECKED!');
		};

		picoCheck.checked = _song.isPico;
		// _song.needsVoices = check_voices.checked;
		picoCheck.callback = function()
		{
			_song.isPico = picoCheck.checked;
			_song.isErect = false;
			_song.isBf = false;
			bfCheck.checked = false;
			erectCheck.checked = false;
			//trace('CHECKED!');
		};

		bfCheck.checked = _song.isBf;
		// _song.needsVoices = check_voices.checked;
		bfCheck.callback = function()
		{
			_song.isBf = bfCheck.checked;
			_song.isPico = false;
			_song.isErect = false;
			picoCheck.checked = false;
			erectCheck.checked = false;
		...
		tab_group_song.add(erectCheck);
		tab_group_song.add(picoCheck);
		tab_group_song.add(bfCheck);
	...
	function loadSong():Void
	{
		...
		else if (_song.isPico)
			file = Paths.voicesPico(currentSongName);
		else if (_song.isBf)
			file = Paths.voicesBf(currentSongName);
	...
	function generateSong() {
		if (_song.isErect){
			FlxG.sound.playMusic(Paths.instErect(currentSongName), 0.6/*, false*/);
		}else if (_song.isPico){
			FlxG.sound.playMusic(Paths.instPico(currentSongName), 0.6/*, false*/);
		}else{ if (_song.isBf){
			FlxG.sound.playMusic(Paths.instBf(currentSongName), 0.6/*, false*/);
		}else
